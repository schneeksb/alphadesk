...
- `git push origin main` → Render (backend) + Vercel (frontend) auto-deploy. Scheduled brief:
  ...
  Market Pulse (`fetch_transcripts.py`) runs LOCALLY on a residential IP (Windows Task Scheduler,
  ~7:50 AM) — YouTube blocks datacenter IPs, so there is no cloud runner for it. Optional transcript
  proxy via `WEBSHARE_USER/PASS` or `YT_PROXY` (see the Market Pulse gotcha above) if the home IP
  is ever blocked.
